...
class TransactionInformationBase {
 public:
  ...
  virtual void unlock(LockManager<common::StdAllocator>* lm) = 0;
...
class WriteTransactionInformation : public TransactionInformationBase<Allocator> {
 public:
  ...
  void unlock(LockManager<common::StdAllocator>* lm);
...
template <typename Value,
          ...
          class List,
          typename Allocator>
void WriteTransactionInformation<Value, ValueVector, Vector, List, Allocator>::unlock(
    LockManager<common::StdAllocator>* lm) {
  lm->unlock(transaction_, reinterpret_cast<void*>(&rw_table_), offset_);
}

template <template <typename> class Vector, template <typename> class List, typename Allocator>
void ReadTransactionInformation<Vector, List, Allocator>::unlock(LockManager<common::StdAllocator>* lm) {
  lm->unlock(transaction_, reinterpret_cast<void*>(&rw_table_), offset_);
}
