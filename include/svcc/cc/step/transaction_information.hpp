...
class TransactionInformationBase {
 public:
  ...
  // virtual void unlock(locking::LockManager<Allocator>* lm) = 0;
...
class WriteTransactionInformation : public TransactionInformationBase<Allocator> {
 public:
  ...
  // void unlock(locking::LockManager<Allocator>* lm);
...
class ReadTransactionInformation : public TransactionInformationBase<Allocator> {
 public:
  ReadTransactionInformation(Vector<List<uint64_t>*>& rw_table, uint64_t lsn, uint64_t offset, uint64_t transaction)
...
/*
template <typename Value, template <typename> class Vector, template <typename> class List, typename Allocator>
void WriteTransactionInformation<Value, Vector, List, Allocator>::unlock(locking::LockManager<Allocator>* lm) {
  lm->unlock(transaction_, reinterpret_cast<void*>(&rw_table_), offset_);
}

template <template <typename> class Vector, template <typename> class List, typename Allocator>
void ReadTransactionInformation<Vector, List, Allocator>::unlock(locking::LockManager<Allocator>* lm) {
  lm->unlock(transaction_, reinterpret_cast<void*>(&rw_table_), offset_);
}*/
...
void WriteTransactionInformation<Value, ValueVector, Vector, List, Allocator>::writeValue(
  ...
  // lsn_column_ also used as locked vector since locked isn't touched in transaction coordinator
  tc.template writeValue<Value, ValueVector, Vector, List, true>(data_prv_, column_, lsn_column_, rw_table_,
                                                                 lsn_column_, offset_, transaction_);
}
