...
class ExtentVector {
 ...
 private:
  ...
  friend class ExtentVectorIterator<Value>;

  using iterator = ExtentVectorIterator<Value>;

 public:
  ExtentVector() : extent_(0), size_(0), lowest_alive_(0), safe_read_(0), mutex_(){};

  ExtentVector(const ExtentVector&) = default;
  ~ExtentVector() {
    for (uint64_t i = 0; i < extent_; i++) {
      auto size = 1ul << (i + reserved_ - 1);
      deallocate(buckets_[i], sizeof(Value) * size);
  ...
  inline Value operator[](const uint64_t n) const {
    ...
    return buckets_[v][off_n];
  }

  inline Value lookup(const uint8_t segment, const uint64_t off_n) const { return buckets_[segment][off_n]; }

  inline Value replace(const uint64_t n, const Value& value) {
    uint8_t v = get_segment_base_offset(n);
    uint64_t off_n = n - get_segment_base(v);
    auto old = buckets_[v][off_n];
    buckets_[v][off_n] = value;
    return old;
  }
  ...
  inline uint64_t push_back(T&& value) {
    ...
    buckets_[v][n] = std::forward<T>(value);  // std::atomic<Value>
    safe_read_++;
    alive_[v][n] = true;
    return new_n;
  }
  ...
  inline void reserve(std::size_t n) {
    ...
    if (extent_ == 0 && n > 0) {
      ...
      buckets_ = new Value*[8]();
      alive_ = new std::atomic<bool>*[8]();

      buckets_[extent_] = new (allocate(sizeof(Value) * n)) Value[n]();
  ...
  inline void resize() {
    ...
    if (size_ >= max_size()) {
      ...
      if (extent_ == 8 || extent_ == 16 || extent_ == 32) {
        auto buckets = new Value*[2 * extent_]();
        auto deleted = new std::atomic<bool>*[2 * extent_]();

        std::memcpy(buckets, buckets_, extent_ * sizeof(Value*));
...
template <typename Value>
class ExtentVectorIterator : public std::iterator<std::bidirectional_iterator_tag, Value> {
  ExtentVector<Value>& vector_;
  uint64_t pos_;
 ...
 public:
  ExtentVectorIterator() : pos_(0), base_offset_(0), base_(0) {}

  ExtentVectorIterator(ExtentVector<Value>& vec, uint64_t pos) : vector_(vec), pos_(pos), end_(false) {
  ...
  ExtentVectorIterator(ExtentVector<Value>& vec, bool end) : vector_(vec), pos_(0), end_(end) {
    if (!end) {
      base_offset_ = vector_.get_segment_base_offset(pos_);
      base_ = pos_ - vector_.get_segment_base(base_offset_);
  ...
  ExtentVectorIterator operator++(int) {
    ExtentVectorIterator tmp(*this);
    operator++();
    return tmp;
  }
  ...
  bool operator==(const ExtentVectorIterator& rhs) const {
    if (!end_ && !rhs.end_)
  ...
  bool operator!=(const ExtentVectorIterator& rhs) const { return !(*this == rhs); }

  Value operator*() const { return vector_.buckets_[base_offset_][base_]; }
};
